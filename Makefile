...
TARGET_NAME			:=	CopyType
# custom flags:
LFLAGS				:= -lcomctl32 -lgdi32 -mwindows
CCXFLAGS			:= -Wall -Wextra -pedantic -DUNICODE
...
RESOURCE_OBJ	:=	$(if $(RESOURCE),$(call DIRSEP,$(OBJECT_DIR))$(RESOURCE).res,)
...
$(TARGET_OUTPUT): $(OBJECTS) $(RESOURCE_OBJ)
...
$(RESOURCE_OBJ):
	@$(if $(AT),,$(NL)$(BR)) $(MESSAGE_2.1)
	$(AT) windres -J rc -i $(RESOURCE).rc -O coff -o $@
...
clean:
	@$(NL)$(BR)
	@$(if $(wildcard $(TARGET_OUTPUT) $(RESOURCE_OBJ) $(OBJECTS) $(DEPS))$(call IS_DIR,$(OBJECT_DIR))$(call IS_DIR,$(TARGET_DIR)),\
		$(if $(wildcard $(TARGET_OUTPUT) $(RESOURCE_OBJ) $(OBJECTS) $(DEPS)),\
			$(MESSAGE_1.0) $(BR) \
			$(MESSAGE_2.4)$" executables:	$"$(call FIXPATH,$(TARGET_OUTPUT)) $(BR) \
			$(call RECIPE_DEBUG, $(RM) $(TARGET_OUTPUT) $(BR)) \
			$(MESSAGE_2.4)$" objects:	$"$(call FIXPATH,$(OBJECTS) $(RESOURCE_OBJ)) $(BR) \
			$(call RECIPE_DEBUG, $(RM) $(call FIXPATH,$(OBJECTS) $(RESOURCE_OBJ)) $(BR)) \
